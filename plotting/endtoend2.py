...
algorithms = ['Dp', 'INFaaS-Accuracy', 'Proteus','Clipper-HT']
...
#画throughput，effective acc，slo violation的三个图
for idx in range(len(logfile_list)):
    logfile = logfile_list[idx]
    
    algorithm = logfile.split('/')[-1].rstrip('.csv')  #获取算法的名称

    df = pd.read_csv(logfile)

    original_df = copy.deepcopy(df)

    aggregated = df.groupby(df.index // 10).sum(numeric_only=True)  #数据按照10个为1组进行聚合，10个的sum成为一个新数据
    aggregated = df.groupby(df.index // 10).mean(numeric_only=True) #同样按10个为一组，10个的mean成为一个新数据
    df = aggregated
    ...
    y_cutoff = max(y_cutoff, max(demand))  #是所有算法demand的最大值
    ...
    if 'infaas' in algorithm:
        ...
        dp_y_intercept=min(effective_accuracy)
